...
InstanceManager::InstanceManager(MainWindow *_w, QObject *parent) :
    QThread(parent) {
    w = _w;
}
...
void InstanceManager::run() {
    ...
    connect(this, SIGNAL(showMainWindow()),w,SLOT(showMainWindow()),Qt::QueuedConnection);
    //connect(this, SIGNAL(showMainWindow()),w,SLOT(()),Qt::QueuedConnection);
